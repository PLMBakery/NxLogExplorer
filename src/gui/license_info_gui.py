# src/gui/license_info_gui.py

import tkinter as tk
from tkinter import messagebox, Toplevel
from parser.license_info_parser import parse_license_info

def add_license_info_button(frame, app):
    license_info_button = tk.Button(frame, text="License Information", command=lambda: show_license_info(app))
    license_info_button.grid(row=0, column=1, padx=10, pady=10)

def show_license_info(app):
    if not hasattr(app, 'file_path') or not app.file_path:
        messagebox.showerror("No file selected", "Please select a log file first.")
        return

    license_info = parse_license_info(app.file_path)
    show_results(app, license_info, "License Information")
